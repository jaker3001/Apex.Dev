...
export function MessageList({ messages, isStreaming }: MessageListProps) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-900">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-2 max-w-xl">
          {[
            ...
            <button
              key={prompt}
              className="px-4 py-2.5 text-left text-sm text-slate-400 hover:text-white border border-slate-700 rounded-lg hover:border-slate-600 hover:bg-slate-800 transition-colors"
            >
              {prompt}
            </button>
          ))}
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto p-6 bg-slate-900">
